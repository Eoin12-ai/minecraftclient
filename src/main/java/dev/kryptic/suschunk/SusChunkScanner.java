...
public class SusChunkScanner {
   ...
   private SusChunkScanner.ChunkScore scanChunk(MinecraftClient client, WorldChunk chunk) {
      ...
      if (DEBUG_LOG && chunkKey2.coord > 0.0) {
         KrypticClient.LOGGER.info("SusChunk: chunk {} scored {} across {} signal hits",
               new Object[]{chunk.getPos(), chunkKey2.coord, chunkKey2.hits});
      }
      ...
      if (!list.isEmpty()) {
         ...
         for (BlockPos pos : list) {
            ...
            if ((state.isAir() || state.isOf(Blocks.AMETHYST_CLUSTER)) && hasAmethystNeighbour(client, pos, mutablePos)) {
               ...
               if (DEBUG_LOG) {
                  KrypticClient.LOGGER.info("SusChunk: amethyst at {} ({})", pos, state.isAir() ? "hidden" : "visible");
               }
            }
         }
      }
   ...
   private void inspectPlant(MinecraftClient client, BlockState state, BlockPos pos, SusChunkScanner.ChunkScore chunkScore) {
      ...
      if ((block == Blocks.KELP || block == Blocks.KELP_PLANT) && this.module.kelp.get()) {
         ...
         if (matches && n >= 8 || n >= 14) {
            ...
            if (DEBUG_LOG) {
               KrypticClient.LOGGER.info("SusChunk: kelp at {}, column {}, fully grown {}", new Object[]{pos, n, matches});
            }
         }
         ...
         if (localZ >= 12) {
            ...
            if (DEBUG_LOG) {
               KrypticClient.LOGGER.info("SusChunk: bamboo at {}, column {}", pos, localZ);
            }
         }
         ...
         if (state.contains(Properties.AGE_3) && (Integer)state.get(Properties.AGE_3) == 3) {
            ...
            if (DEBUG_LOG) {
               KrypticClient.LOGGER.info("SusChunk: fully grown berry bush at {}", pos);
            }
         }
         ...
         if (localY >= 7) {
            ...
            if (DEBUG_LOG) {
               KrypticClient.LOGGER.info("SusChunk: vines at {}, hanging {}", pos, localY);
            }
         }
         ...
         if (step >= 5) {
            ...
            if (DEBUG_LOG) {
               KrypticClient.LOGGER.info("SusChunk: dripstone at {}, spike {}", pos, step);
            }
         }
      }
   }
         ...
         if (!(geode.totalScore() < (double)n)) {
            ...
            if (DEBUG_LOG) {
               KrypticClient.LOGGER
                  .info(
                     "SusChunk: zone of {} chunks ({} members) totalling {}",
