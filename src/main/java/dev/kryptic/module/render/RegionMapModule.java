package dev.kryptic.module.render;

import dev.kryptic.module.Category;
import dev.kryptic.module.Module;
import dev.kryptic.settings.BooleanSetting;
import dev.kryptic.settings.SliderSetting;
import net.minecraft.client.MinecraftClient;
import net.minecraft.client.network.ClientPlayerEntity;

public class RegionMapModule extends Module {
   public static final int MAP_SIZE = 9;
   private static final double REGION_SIZE = 50000.0;
   private static final double MAP_OFFSET = 225000.0;
   private static final int[] TYPE_COLORS = new int[]{
      rgb(159, 206, 99), rgb(0, 166, 99), rgb(79, 173, 234), rgb(47, 110, 186), rgb(245, 194, 66), rgb(252, 136, 3)
   };
   private static final int[][] LAYOUT = new int[][]{
      {82, 5},
      {100, 3},
      {101, 3},
      {102, 3},
      {103, 2},
      {104, 2},
      {105, 2},
      {106, 2},
      {91, 2},
      {83, 5},
      {44, 3},
      {75, 3},
      {42, 3},
      {41, 2},
      {40, 2},
      {39, 2},
      {38, 2},
      {92, 2},
      {84, 5},
      {45, 3},
      {14, 3},
      {13, 3},
      {12, 2},
      {11, 2},
      {10, 2},
      {37, 2},
      {93, 2},
      {85, 5},
      {46, 5},
      {74, 5},
      {3, 3},
      {2, 2},
      {1, 2},
      {25, 2},
      {36, 2},
      {94, 2},
      {86, 4},
      {47, 4},
      {72, 4},
      {71, 4},
      {5, 2},
      {4, 2},
      {24, 2},
      {35, 2},
      {95, 2},
      {87, 4},
      {51, 1},
      {17, 1},
      {9, 0},
      {8, 0},
      {7, 0},
      {23, 0},
      {34, 0},
      {96, 2},
      {88, 4},
      {54, 1},
      {18, 1},
      {61, 0},
      {62, 0},
      {21, 0},
      {22, 0},
      {33, 0},
      {97, 0},
      {89, 0},
      {26, 1},
      {27, 0},
      {28, 0},
      {29, 0},
      {30, 0},
      {59, 0},
      {32, 0},
      {98, 0},
      {90, 0},
      {107, 1},
      {108, 1},
      {109, 1},
      {110, 1},
      {111, 1},
      {112, 1},
      {113, 1},
      {99, 0}
   };
   private static final int[] REGION_ID = new int[81];
   private static final int[] REGION_TYPE = new int[81];
   private static final String[] TYPE_NAMES = new String[]{"Spawn", "Nether Hub", "Wilderness", "Ocean", "Mountain", "Desert"};
   public final SliderSetting opacity = this.addSetting(new SliderSetting("Opacity", "Region cell fill opacity.", 90.0, 10.0, 100.0, 5.0, "V"));
   public final BooleanSetting gridLines = this.addSetting(new BooleanSetting("Grid Lines", "Draw the accent lines between region cells.", true));
   public final BooleanSetting cellNumbers = this.addSetting(new BooleanSetting("Cell Numbers", "Show each region's ID number in its cell.", true));
   public final BooleanSetting legend = this.addSetting(new BooleanSetting("Legend", "Show the region-type colour legend below the grid.", true));

   public RegionMapModule() {
      super("Region Map", "DonutSMP server region map on the HUD.", Category.RENDER);
   }

   private static int rgb(int n, int localX, int localZ) {
      return n << 16 | localX << 8 | localZ;
   }

   public boolean hasData() {
      return true;
   }

   public int mapSize() {
      return 9;
   }

   public int regionTypeCount() {
      return TYPE_NAMES.length;
   }

   public String regionTypeName(int n) {
      return n >= 0 && n < TYPE_NAMES.length ? TYPE_NAMES[n] : "";
   }

   public int regionTypeRgb(int n) {
      return n >= 0 && n < TYPE_COLORS.length ? TYPE_COLORS[n] : 16777215;
   }

   public int regionTypeAt(int n) {
      return n >= 0 && n < REGION_TYPE.length ? REGION_TYPE[n] : -1;
   }

   public int regionIdAt(int n) {
      return n >= 0 && n < REGION_ID.length ? REGION_ID[n] : -1;
   }

   public int[] worldToGrid(double d, double coord) {
      int n = (int)Math.floor((d + 225000.0) / 50000.0);
      int offset = (int)Math.floor((coord + 225000.0) / 50000.0);
      return new int[]{n, offset};
   }

   public double[] worldToCellPosition(double d, double coord) {
      double currentScore = (d + 225000.0) % 50000.0 / 50000.0;
      double coord3 = (coord + 225000.0) % 50000.0 / 50000.0;
      if (currentScore < 0.0) {
         currentScore++;
      }

      if (coord3 < 0.0) {
         coord3++;
      }

      return new double[]{Math.clamp(currentScore, 0.0, 1.0), Math.clamp(coord3, 0.0, 1.0)};
   }

   public int regionIdAtWorld(double d, double coord) {
      int[] n = this.worldToGrid(d, coord);
      return n[0] >= 0 && n[0] < 9 && n[1] >= 0 && n[1] < 9 ? this.regionIdAt(n[1] * 9 + n[0]) : -1;
   }

   public int regionTypeAtWorld(double d, double coord) {
      int[] n = this.worldToGrid(d, coord);
      return n[0] >= 0 && n[0] < 9 && n[1] >= 0 && n[1] < 9 ? this.regionTypeAt(n[1] * 9 + n[0]) : -1;
   }

   public int currentRegionId() {
      ClientPlayerEntity temp = MinecraftClient.getInstance().player;
      return temp == null ? -1 : this.regionIdAtWorld(temp.getX(), temp.getZ());
   }

   static {
      for (int n = 0; n < LAYOUT.length && n < REGION_ID.length; n++) {
         if (LAYOUT[n].length >= 2) {
            REGION_ID[n] = LAYOUT[n][0];
            REGION_TYPE[n] = Math.min(LAYOUT[n][1], TYPE_NAMES.length - 1);
         } else {
            REGION_TYPE[n] = -1;
         }
      }
   }
}
