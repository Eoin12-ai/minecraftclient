   ...
   public void onTick() {
      ...
      if (client.player != null && client.interactionManager != null) {
         if (this.waitingToSwapBack) {
            ...
            boolean pressed = GLFW.glfwGetMouseButton(l, 0) == 1;
            if (pressed
               && client.currentScreen == null
               && client.crosshairTarget != null
               && client.crosshairTarget.getType() == Type.ENTITY
               && !(client.player.getAttackCooldownProgress(0.5F) < 1.0F)
               && (!this.onlySword.get() || this.isSword(client.player.getMainHandStack().getItem()))
               && (!this.onlyAxe.get() || client.player.getMainHandStack().getItem() instanceof AxeItem)
               && !client.player.getMainHandStack().isOf(Items.MACE)
               && InventoryHelper.getHotbarSlot(Items.MACE) != -1) {
