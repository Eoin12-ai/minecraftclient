...
public class DoubleAnchorModule extends Module {
   ...
   public void onTick() {
      ...
      if (client.player != null && client.interactionManager != null && client.world != null && client.currentScreen == null) {
         ...
         } else if (this.keyDown(client) && client.crosshairTarget instanceof BlockHitResult blockHit && blockHit.getType() == Type.BLOCK) {
   ...
   private void processStep(MinecraftClient client) {
      ...
      if (client.crosshairTarget instanceof BlockHitResult blockHit2) {
         blockHit = blockHit2;
      } else {
         blockHit = null;
      }
      ...
      switch (this.step) {
         case 1:
            if (!this.lookingAtTarget(blockHit)) {
               this.resetState();
               return;
            }
         ...
         case 4:
            if (BlockHelper.isBlockAt(this.targetPos, Blocks.RESPAWN_ANCHOR)) {
               if (this.lookingAtTarget(blockHit) && BlockHelper.isAnchorUncharged(this.targetPos)) {
                  InventoryHelper.swapToItem(Items.GLOWSTONE);
                  BlockHelper.interactBlock(blockHit, true);
               }

               this.advance(5);
            } else if (this.rePlace.get() && blockHit != null && InventoryHelper.getHotbarSlot(Items.RESPAWN_ANCHOR) >= 0) {
               InventoryHelper.swapToItem(Items.RESPAWN_ANCHOR);
               BlockHelper.interactBlock(blockHit, true);
               this.targetPos = blockHit.getBlockPos().offset(blockHit.getSide()).toImmutable();
               this.advance(6);
            } else {
               this.finish(client);
            }
            break;
         case 5:
            if (this.lookingAtTarget(blockHit) && BlockHelper.isBlockAt(this.targetPos, Blocks.RESPAWN_ANCHOR)) {
               InventoryHelper.swap(this.detonateSlot.getInt() - 1);
               BlockHelper.interactBlock(blockHit, true);
            }
   ...
   private boolean lookingAtTarget(BlockHitResult blockHit) {
      return blockHit != null && blockHit.getType() == Type.BLOCK && blockHit.getBlockPos().equals(this.targetPos);
   }
   ...
   private boolean keyDown(MinecraftClient client) {
      ...
      if (n == -1) {
         ...
         return n <= 7 ? GLFW.glfwGetMouseButton(l, n) == 1 : GLFW.glfwGetKey(l, n) == 1;
      }
   }
