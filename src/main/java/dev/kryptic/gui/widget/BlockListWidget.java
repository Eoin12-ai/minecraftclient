...
public class BlockListWidget extends SettingWidget {
   ...
   public BlockListWidget(ThemeManager themeManager, BlockListSetting blockListSetting) {
      super(themeManager, blockListSetting);
   ...
   public boolean mouseClicked(float f, float f3, int n) {
      if (n == 0 && this.contains(f, f3)) {
         if (MinecraftClient.getInstance().currentScreen instanceof ClickGuiScreen clickGuiScreen) {
            clickGuiScreen.openBlockPicker(this.setting);
            UiSounds.select();
            return true;
         } else {
            return false;
