   ...
   private static void ensureLoaded() {
      if (!loaded) {
         ...
         for (Category category : Category.values()) {
            int n = loadSvg("assets/sixsevenclient/icons/" + category.getIconId() + ".svg");
            if (n > 0) {
   ...
   private static int loadSvg(String str) {
      ...
      try {
         ...
         try (InputStream inputStream = NVGIcons.class.getClassLoader().getResourceAsStream(str)) {
            if (inputStream == null) {
               SixSevenClient.LOGGER.warn("Icon resource missing: {}", str);
               return -1;
            }
         ...
         SixSevenClient.LOGGER.error("Failed to rasterize icon {}", str, ex);
         return -1;
      }
   }
}
