   ...
   public static void render(Immediate immediate, MatrixStack matrices, Vec3d vec, JumpCirclesModule jumpCirclesModule) {
      ...
      if (!deque.isEmpty()) {
         ...
         if (!deque.isEmpty()) {
            ...
            VertexConsumer consumer = immediate.getBuffer(RenderLayers.entityTranslucentEmissive(TEXTURE_67));
            ...
            if (jumpCirclesModule.shockwave.get()) {
               VertexConsumer consumer2 = immediate.getBuffer(RenderLayers.entityTranslucentEmissive(TEXTURE_RING));
