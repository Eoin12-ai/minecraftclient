...
import dev.sixseven.discord.PresenceState;
...
public class DiscordPresenceModule extends Module {
    ...
    private PresenceState build() {
        ...
        return new PresenceState(
                ...
                start,
                largeImage.get(),
                SixSevenClient.NAME + " " + SixSevenClient.VERSION,
                "",
                "");
    }
    ...
    private String expand(String template, MinecraftClient client) {
        ...
        if (out.contains("{client}"))    out = out.replace("{client}", SixSevenClient.NAME);
        if (out.contains("{version}"))   out = out.replace("{version}", SixSevenClient.VERSION);
        return out;
    }
